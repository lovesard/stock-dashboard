...
def get_finnhub_quote(symbol: str, api_key: str):
 ...
 return None
 try:
 url = fhttps://finnhub.io/api/v1/quote?symbol={symbol}&token={api_key}
 ...
 print(fFinnhub API Error ({symbol}): {e})
 return None
...
if FINNHUB_API_KEY:
 ...
 st.warning(⚠️ .env 또는 Streamlit Secrets에 FINNHUB_API_KEY를 등록하면 미국 실시간 시세가 활성화됩니다.)
...
st.sidebar.caption(f선택된 티커: **{ticker_input}**)
...
with col_title:
 ...
 st.caption(f구분: {stock_type_str} | 통화: {currency} | 기준 분석 기간: {selected_period_label})
with col_time:
...
def get_tradingview_symbol(ticker: str):
 ...
 if t.endswith(.KS):
 return fKRX:{t.replace('.KS', '')}
 elif t.endswith(.KQ):
 return fKOSDAQ:{t.replace('.KQ', '')}
 elif t == BTC-USD:
...
with tab_quant:
 ...
 st.write(f현재가: {format_price(latest_close)} / 20일선: {format_price(ma20_val)})
 ...
 st.write(f20일선: {format_price(ma20_val)} / 60일선: {format_price(ma60_val)})
